...
-- Global Better PC Dragging Logic
...
local ConfigFileName = "IanVz_Duels_Config_V3.json"
...
local function toggleSpeedMeter(state)
    ...
    if state then
        ...
        speedMeterGui.Name = "IanVzSpeedMeter"
...
coordESPFolder.Name = "IanVz_CoordESP"
...
radiusVisualizer.Name = "IanVzRadiusVisualizer"
...
local function createAutoStealUI()
    ...
    autoStealGui.Name = "IanVzAutoStealUI"
...
local function createESP(plr)
    ...
    if not hrp or char:FindFirstChild("IanVzHitbox") then return end
...
local function toggleESP(state)
    if not state then
        for _, p in ipairs(Players:GetPlayers()) do 
            if p.Character then 
                local hb = p.Character:FindFirstChild("IanVzHitbox")
                local nm = p.Character:FindFirstChild("IanVzName")
...
-- Mobile Buttons Logic (Using MouseButton1Click as requested)
if isMobile then
...
-- Startup & Respawn Logic Overhaul
task.spawn(function()
    task.wait(2)
    ...
    -- Restore Theme and BG from config
    local tName = ThemeNames[CurrentThemeIndex]
    if isRainbow then
